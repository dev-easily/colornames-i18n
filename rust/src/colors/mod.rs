...
pub mod javascript;
// pub use javascript::*;
pub mod json_separated;
// pub use json_separated::*;
pub mod types;
// pub use types::*;
...
pub struct Args {
    ...
    #[arg(short, long, default_value = "sources/colornames.i18n.csv")]
...
pub fn update(origin: &Path, i18n: &Path) -> anyhow::Result<()> {
    let origin_colors = colors::file::read_color_names_from_origin_csv(origin);
    let mut translated = HashMap::new();

    let binding = colors::file::read_color_names_from_i18n_csv(i18n);
    binding
        .iter()
        .for_each(|color| {
    ...
    let head = colors::file::read_csv_headers_from_i18n_csv(i18n);
    let heads = head.split(",").map(|x| x.trim()).collect::<Vec<&str>>();

    let mut updated_file = colors::file::init_target_file(i18n, "updated.csv").expect("failed to init updated.csv, check to see if directory exists");
